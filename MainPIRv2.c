#define LED			P1_4
#define SW2			P0_6	// Magnetic switch
...
#define T_REPORT	25	// default = 2minutes or 120 (1secs *120)
...
unsigned char swPressedSW2 (void)
{
	bit swVal;
	SW_Pull=1;
	delayms(1);
	...
	{
		swValOldSW2 = swVal;
		SW_Pull=0;//delayms(10);
		return 0x10;
	}
	else if(swValOldSW2 == 0 && swVal == 1)
	{
		swValOldSW2 = swVal;
		SW_Pull=0;//delayms(10);
		return 0x10;
	}
	else if(swValOldSW2 == 0 && swVal == 0)
	{
		swValOldSW2 = swVal;
		SW_Pull=0;//delayms(10);
		return 0x00;
     	...
     	{	
			...
			{	
				if (iCount == T_REPORT)	// Every 3 minutes
				{
					...
					Tx_Buff[2] = nodeByte | swPressedSW2();
					Tx_Buff[1] = adcGet() >> 2;
					Tx_Buff[0] = getCheckSum(Tx_Buff[1], Tx_Buff[2], NODE_CTRL);
